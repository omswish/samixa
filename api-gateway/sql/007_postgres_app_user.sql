...
CREATE INDEX IF NOT EXISTS idx_app_user_role_enabled
  ON app_user (role, enabled);
...
CREATE INDEX IF NOT EXISTS idx_app_user_audit_email_time
  ON app_user_audit (email, changed_at DESC);

CREATE OR REPLACE FUNCTION set_app_user_timestamps()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_app_user_timestamps ON app_user;
CREATE TRIGGER trg_app_user_timestamps
BEFORE INSERT OR UPDATE ON app_user
FOR EACH ROW
EXECUTE FUNCTION set_app_user_timestamps();
...
FOR EACH ROW
EXECUTE FUNCTION audit_app_user_change();
